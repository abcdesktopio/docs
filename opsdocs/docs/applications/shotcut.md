# Shotcut
![circle_shotcut.svg](icons/circle_shotcut.svg){: style="height:64px;width:64px"}
## inherite from
...
``` json
{
    ...
    "desktopfile": "/usr/share/applications/shotcut.desktop",
...
``` sh
ABCHOST=localhost
curl --output Shotcut.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Shotcut.d.3.0.json
...
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends shotcut && apt-get clean
...
LABEL oc.desktopfile="shotcut.desktop"
