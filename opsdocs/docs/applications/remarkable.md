# remarkable
![remarkable.svg](icons/remarkable.svg){: style="height:64px;width:64px"}
## inherite from
...
```
RUN apt-get update && apt-get install --no-install-recommends --yes python3-gtkspellcheck wkhtmltopdf python3-markdown gir1.2-gtksource-3.0 yelp gir1.2-webkit-3.0 python3-bs4 && apt-get clean
...
``` sh
ABCHOST=localhost
curl --output remarkable.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/remarkable.d.3.0.json
