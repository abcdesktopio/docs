...
``` json
{
    ...
    "apkpackage": "krb5-user winbind firefox",
...
``` sh
ABCHOST=localhost
curl --output Firefox.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Firefox.d.3.0.json
...
COPY /ntlm_auth /usr/bin/ntlm_auth.desktop
RUN chown root:root /usr/bin/ntlm_auth.desktop && chmod 111 /usr/bin/ntlm_auth.desktop
RUN apk add --no-cache --update krb5-user winbind firefox
