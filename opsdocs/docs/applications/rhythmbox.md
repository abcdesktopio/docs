# rhythmbox
![circle_rhythmbox.svg](icons/circle_rhythmbox.svg){: style="height:64px;width:64px"}
## inherite from
...
``` json
{
    ...
    "desktopfile": "/usr/share/applications/rhythmbox.desktop"
}
```
...
``` sh
ABCHOST=localhost
curl --output rhythmbox.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/rhythmbox.d.3.0.json
...
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends rhythmbox && apt-get clean
...
LABEL oc.desktopfile="rhythmbox.desktop"
...
[Dockerfile for application rhythmbox](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/rhythmbox.d)
``` sh
