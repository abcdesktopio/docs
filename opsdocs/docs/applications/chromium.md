# chromium
![circle_chromium.svg](icons/circle_chromium.svg){: style="height:64px;width:64px"}
## inherite from
...
``` json
{
    ...
    "cat": "office",
    "debpackage": "krb5-user fonts-noto fonts-roboto xfonts-100dpi fonts-ubuntu fonts-freefont-ttf dbus-x11 fonts-wine chromium-browser",
    "icon": "circle_chromium.svg",
    "keyword": "web,browser,internet",
    "launch": "chromium-browser.Chromium-browser",
    "name": "chromium",
    "rules": {
        "homedir": {
            "default": true
...
``` sh
ABCHOST=localhost
curl --output chromium.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/chromium.d.3.0.json
...
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y krb5-user fonts-noto fonts-roboto xfonts-100dpi fonts-ubuntu fonts-freefont-ttf dbus-x11 fonts-wine chromium-browser && apt-get clean
...
LABEL oc.launch="chromium-browser.Chromium-browser"
...
[Dockerfile for application chromium](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/chromium.d)
``` sh
