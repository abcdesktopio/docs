...
[Dockerfile for application cmd.exe](cmd.exe.d)
### build this `Dockerfile` to create a container image
``` sh
docker build --build-arg TAG=3.0 -f cmd.exe -t cmd.exe .
```
...
curl -X PUT -H 'Content-Type: text/javascript' http://localhost:30443/API/manager/image -d @cmd.exe.json
```
...
LABEL oc.keyword="cmd.exe,cmd.exe"
