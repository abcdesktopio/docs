...
``` sh
ABCHOST=localhost
curl --output Mines.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Mines.d.3.0.json
...
crets/abcdesktop/localaccount/shadow /etc/shadow
RUN rm -f /etc/gshadow && ln -s /var/secrets/abcdesktop/localaccount/gshadow /etc/gshadow
USER balloon
CMD [ "/composer/appli-docker-entrypoint.sh" ]
