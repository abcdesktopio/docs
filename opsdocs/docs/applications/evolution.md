...
[Dockerfile for application evolution](evolution.d)
### build this `Dockerfile` to create a container image
``` sh
docker build --build-arg TAG=3.0 -f evolution -t evolution .
```
...
curl -X PUT -H 'Content-Type: text/javascript' http://localhost:30443/API/manager/image -d @evolution.json
```
...
LABEL oc.cat="office"
LABEL oc.desktopfile="evolution.desktop"
