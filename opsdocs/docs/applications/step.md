# Step
![step.svg](icons/step.svg){: style="height:64px;width:64px"}
## inherite from
...
``` sh
ABCHOST=localhost
curl --output Step.json https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Step.d.3.0.json
...
USER root
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y  --no-install-recommends step && apt-get clean
...
[Dockerfile for application Step](https://raw.githubusercontent.com/abcdesktopio/oc.apps/main/Step.d)
``` sh
