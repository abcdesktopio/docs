...
## Install `NVIDIA_CONTAINER_TOOLKIT=1.18.1-1`

Install the `NVIDIA_CONTAINER_TOOLKIT=1.18.1-1`
...
4 upgraded, 0 newly installed, 0 to remove and 25 not upgraded.
Need to get 0 B/7 656 kB of archives.
After this operation, 5 621 kB of additional disk space will be used.
(Reading database ... 388399 files and directories currently installed.)
Preparing to unpack .../libnvidia-container1_1.18.1-1_amd64.deb ...
Unpacking libnvidia-container1:amd64 (1.18.1-1) over (1.17.8-1) ...
Preparing to unpack .../libnvidia-container-tools_1.18.1-1_amd64.deb ...
Unpacking libnvidia-container-tools (1.18.1-1) over (1.17.8-1) ...
Preparing to unpack .../nvidia-container-toolkit_1.18.1-1_amd64.deb ...
Unpacking nvidia-container-toolkit (1.18.1-1) over (1.17.8-1) ...
Preparing to unpack .../nvidia-container-toolkit-base_1.18.1-1_amd64.deb ...
Unpacking nvidia-container-toolkit-base (1.18.1-1) over (1.17.8-1) ...
...
Check if `enable_cdi` option is set to `true` in the new generated files `/etc/containerd/config.toml` and in `/etc/containerd/conf.d/99-nvidia.toml`

```
grep enable_cdi /etc/containerd/config.toml /etc/containerd/conf.d/99-nvidia.toml
```
...
/etc/containerd/conf.d/99-nvidia.toml:    enable_cdi = true
```
...
```
Warning: container debugger-g5zsh: failed to create containerd task: failed to create shim task: OCI runtime create failed: runc create failed: unable to start container process: error during container init: exec: "nvidia-smi": executable file not found in $PATH
```
If you get this error message, the runtime doesn't provide the `nvidia-smi` binary. This is bad 
