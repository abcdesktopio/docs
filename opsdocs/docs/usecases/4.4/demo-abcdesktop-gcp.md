...
Edit `od.config` file and go to the `AUTH SECTION` and empty all dictionaries inside authmanager.
...
Now you cannot authenticate anymore, so you need to add external providers using Oauth2.0 protocol. To proceed, please follow our documentation on [how to add external authentication providers to abcdesktop](https://www.abcdesktop.io/advanced/{{ abcdesktop.latest_release }}/authentication/authexternal/)
