...
This document specifies the runtime behavior of the file service implemented in [file-service.js](file-service.js). It is an implementation-driven specification (what the service does now), not an aspirational API design.
...
- Single Node.js process, Express app and router initialization in [file-service.js](file-service.js#L162).
- Service bind is delegated to external helper `listenDaemonOnContainerIpAddr` in [file-service.js](file-service.js#L24) and [file-service.js](file-service.js#L533).
- All uncaught exceptions are logged but do not trigger explicit shutdown logic in [file-service.js](file-service.js#L529).

### 2.2 Router mounting

- Router is mounted under a regex prefix in [file-service.js](file-service.js#L527):
  - /filer
  - /printerfiler
...
1. `helmet()` security headers in [file-service.js](file-service.js#L165).
2. `express.json()` body parser in [file-service.js](file-service.js#L167).
3. Request logger in [file-service.js](file-service.js#L168).

Route-specific validation middleware comes from [middlewares.js](middlewares.js):
- Query validator for `file`: [middlewares.js](middlewares.js#L15).
- Body validator for `file`: [middlewares.js](middlewares.js#L25).
- Query validator for `directory`: [middlewares.js](middlewares.js#L35).
- Multipart uploaded file structural validator: [middlewares.js](middlewares.js#L45).

Error handling:
- Per-route async exceptions are wrapped by `express-async-handler`.
- Global error handler returns 500 with fixed payload in [file-service.js](file-service.js#L520).

## 3. Configuration Specification

### 3.1 Environment variables

- Root directory: `HOME` read in [file-service.js](file-service.js#L39).
- Listen port: `FILE_SERVICE_TCP_PORT` fallback 29783 in [file-service.js](file-service.js#L40).
- Feature toggles:
  - `SENDFILE` in [file-service.js](file-service.js#L41).
  - `ACCEPTFILE` in [file-service.js](file-service.js#L42).
  - `ACCEPTLISTFILE` in [file-service.js](file-service.js#L43).
  - `ACCEPTDELETEFILE` in [file-service.js](file-service.js#L44).

### 3.2 Toggle interpretation

`is_allow_var` in [file-service.js](file-service.js#L58) maps values as follows:
...
- `checkSafePath` in [file-service.js](file-service.js#L87).
- `normalize_tildpath` in [file-service.js](file-service.js#L67).
...
- Multer is configured with in-memory storage in [file-service.js](file-service.js#L33).
- Uploaded file content is fully resident in RAM before write.

### 5.2 File streaming

- File download uses stream pipeline in [file-service.js](file-service.js#L234).
- Zip download uses JSZip node stream in [file-service.js](file-service.js#L256).

### 5.3 Zip generation

- Recursive traversal implemented by `generateZipTree` in [file-service.js](file-service.js#L141).
...
- Directory list is sorted ascending by modification time in [file-service.js](file-service.js#L117).
...
- [file-service.js](file-service.js#L206)
...
Implementation references:
- [file-service.js](file-service.js#L286)
...
Implementation references:
- [file-service.js](file-service.js#L364)
...
Implementation references:
- [file-service.js](file-service.js#L475)

### 6.5 Fallback route

- Any unmatched route under mounted prefixes returns:
  - `{ "code": 404, "data": "Can not <METHOD> <PATH>" }`
- Implementation at [file-service.js](file-service.js#L509).

## 7. Validation Specification

Validation source: [middlewares.js](middlewares.js).
...
From [__test__/file-service.test.js](__test__/file-service.test.js):
