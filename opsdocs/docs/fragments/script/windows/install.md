...
```PowerShell
$script = curl https://raw.githubusercontent.com/abcdesktopio/conf/main/kubernetes/install-{{ version }}.ps1
...
writing RSA key
[OK] abcdesktop_jwt_des ktop_signing keys create
writing RSA key
...
## Install with a dedicated namespace

You may need to replace the default namespace `abcdesktop` by your own. The `install-{{ version }}.ps1` PowerShell script allows you to set the new namespace as an option.

```PowerShell
curl https://raw.githubusercontent.com/abcdesktopio/conf/main/kubernetes/install-{{ version }}.ps1 -OutFile install-{{ version }}.ps1
```

Run `install-{{ version }}.ps1`

```PowerShell
.\install-{{ version }}.ps1 --namespace superdesktop
```
