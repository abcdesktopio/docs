...
```bash
curl -sL https://raw.githubusercontent.com/abcdesktopio/conf/main/kubernetes/uninstall-{{ version }}.sh | bash
```
...
```bash
wget https://raw.githubusercontent.com/abcdesktopio/conf/main/kubernetes/uninstall-{{ version }}.sh
chmod 755 uninstall-{{ version }}.sh
```

Run the `uninstall-{{ version }}.sh` command line with your own namespace
